...
root=tk.Frame(root1)
root.pack()
root1.title('Market Basket Analysis and Product Recommendation')
root1.geometry('1440x720')
...
def time():     #function to display date and time
    ...
    for i in dateV:
        ...
        DT+='/'
    DT=DT[:len(DT)-1]
    date=tk.Label(root,bg='#e8dc80',text='Date:  '+DT+'\t\tTime:  '+str(q),bd=3, font=('Monseratt'))
...
def addItem():      #add extra item to the bill
    ...
    productE=tk.Label(root,bg='#e8dc80',text=str(pro),relief='flat',width=50)
    productE.grid(column=40,row=row)
    priceE=tk.Label(root,bg='#e8dc80',text=str(pri),relief='flat')
    priceE.grid(column=80,row=row)
    quantityE=tk.Label(root,bg='#e8dc80',text=str(qua),relief='flat')
    quantityE.grid(column=120,row=row)
    productE.destroy()
    priceE.destroy()
    quantityE.destroy()

    SNo=tk.Label(root,bg='#e8dc80',text=str(num))
    ...
    quantityE.grid(column=120,row=row)
    amountE=tk.Label(root,bg='#e8dc80',text=str(amo),relief='flat')
    amountE.grid(column=200,row=row-1)
    Total=tk.Label(root,bg='#e8dc80',text="Total Amount:"+str(sum(totalAmount)),fg='Red')
...
def Submit():
    ...
    #print(dataset)
    top=tk.Toplevel()
    lab=tk.Label(top,text='Submitted Successfully!',font=('Montserrat',15),fg='black')
    lab.pack()
    top.mainloop()
def recommend():
        ...
        def OnDouble(self, event):
            ...
            try:
                ...
                #label=tk.Label(roo,text=str(value[0])+'+'+str(value[1])+' = Rs.'+str(int(Rec_pric)),font=('Tahoma',30),fg='white',bg='black')
                  ...
                  label=tk.Label(roo,text='Something went wrong!',font=('Montserrat',30),fg='white',bg='black')
                  label.pack()
                  roo.mainloop()
...
#title
heading=tk.Label(root,text='Product Billing',font=('Montserrat',50),fg='#0095ff',bg='#e8dc80',bd=3)
heading.grid(column=50,row=2)

#customer Details
nameL=tk.Label(root,bg='#e8dc80',text='Name', font=('Monseratt'))
nameL.grid(column=2,row=4)
nameE=AutocompleteEntry(lista,root)
nameE.grid(column=40,row=4)
NumberL=tk.Label(root,bg='#e8dc80',text='Mobile Number', font=('Monseratt'))
...
SNO=tk.Label(root,bg='#e8dc80',text='SNo.', font=('Monseratt'))
SNO.grid(column=40,row=7)
product=tk.Label(root,bg='#e8dc80',text='Product', font=('Monseratt'))
product.grid(column=40,row=7)
price=tk.Label(root,bg='#e8dc80',text='Price', font=('Monseratt'))
price.grid(column=80,row=7)
quantity=tk.Label(root,bg='#e8dc80',text='Quantity', font=('Monseratt'))
quantity.grid(column=120,row=7)
amount=tk.Label(root,bg='#e8dc80',text='Amount', font=('Monseratt'))
amount.grid(column=200,row=7)

#product that is purchased by customer to be entered by the 
SNo=tk.Label(root,bg='#a1a1a1',text=str(num), font=('Monseratt'))
...
amountE=tk.Label(root,bg='#e8dc80',relief='flat')
amountE.grid(column=200,row=9)

addButton=tk.Button(root,text='Add+',font=('Montserrat'),width=20,command=addItem,bg='#a1a1a1',fg='Black',bd=5)
addButton.grid(column=200,row=30)

SubmitButton=tk.Button(root,text='Submit',font=('Montserrat'),width=20,command=Submit,bg='#a1a1a1',fg='Black',bd=5)
SubmitButton.grid(column=200,row=32)

PrintButton=tk.Button(root,text='Print Bill',font=('Montserrat'),width=20,command=print_bill,bg='#a1a1a1',fg='Black',bd=5)
PrintButton.grid(column=200,row=34)

SubmitButton=tk.Button(root,text='Recommend',font=('Montserrat'),width=20,command=recommend,bg='#a1a1a1',fg='Black',bd=5)
...
root.configure(background='#e8dc80')
root1.configure(background='#e8dc80')
root1.mainloop()
